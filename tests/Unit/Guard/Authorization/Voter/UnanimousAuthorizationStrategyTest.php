...
use Plexikon\Auth\Guard\Authorization\Strategy\UnanimousAuthorizationStrategy;
...
class UnanimousAuthorizationStrategyTest extends TestCase
{
    /**
     * @test
     */
    public function it_grant_vote(): void
    {
        ...
        $auth = new UnanimousAuthorizationStrategy($voters->reveal(), $allowIfAllAbstain);
     ...
     * @test
     */
    public function it_deny_access_on_first_negative_voter(): void
    {
        $allowIfAllAbstain = false;
        $subject = new \stdClass();
        $token = $this->prophesize(Tokenable::class)->reveal();
